<a id="stormcrawler-apache-org-docs-3-5-1-index"></a>
...
- Actively developed and used in production by [multiple organizations](#stormcrawler-apache-org-docs-3-5-1-index--poweredby)
...
|  | In a Storm topology defined with Flux, parallelism specifies the number of tasks or instances of a spout or bolt to run concurrently, enabling scalable and efficient processing. In the archetype every component is set to a parallelism of1. |
| --- | --- |
...
![StormCrawler Architecture](stormcrawler.apache.org/docs/3.5.1/images/stormcrawler.drawio.jpg)
...
Both use the same [Protocols](#stormcrawler-apache-org-docs-3-5-1-index--protocols) implementations and [URLFilters](#stormcrawler-apache-org-docs-3-5-1-index--urlfilters) to control the redirections.
...
This parser calls the [URLFilters](#stormcrawler-apache-org-docs-3-5-1-index--urlfilters) and [ParseFilters](#stormcrawler-apache-org-docs-3-5-1-index--parsefilters) defined in the configuration. Please note that it calls [MetadataTransfer](#stormcrawler-apache-org-docs-3-5-1-index--metadatatransfer) prior to calling the [ParseFilters](#stormcrawler-apache-org-docs-3-5-1-index--parsefilters). If you create new Outlinks in your <a id="stormcrawler-apache-org-docs-3-5-1-index--ParseFilters"></a>, you’ll need to make sure that you use MetadataTransfer there to inherit the Metadata from the parent document.
...
The reason for this is that the **SiteMapParserBolt** acts as a filter: it passes on any incoming tuples to the default stream so that they get processed by the **ParserBolt**, unless the tuple contains `isSitemap=true` in its metadata, in which case the **SiteMapParserBolt** will parse it itself. Any outlinks found in the sitemap files are then emitted on the <a id="stormcrawler-apache-org-docs-3-5-1-index--StatusStream"></a>.

The **SiteMapParserBolt** applies any configured [ParseFilters](#stormcrawler-apache-org-docs-3-5-1-index--parsefilters) to the documents it parses and, just like its equivalent for HTML pages, it uses [MetadataTransfer](#stormcrawler-apache-org-docs-3-5-1-index--metadatatransfer) to populate the Metadata objects for the Outlinks it finds.
...
- The `configure` method takes a JSON object loaded by the wrapper class ParseFilters. The Storm configuration map can also be used to configure the filters, as described in [Configuration](Configuration).
...
See <a id="stormcrawler-apache-org-docs-3-5-1-index--HTTPProtocol"></a> for the effect of metadata content on protocol behaviour.
...
which takes as parameter the standard Storm <a id="stormcrawler-apache-org-docs-3-5-1-index--Configuration"></a>.
...
- **OWler: preliminary results for building a collaborative open web crawler**, Dinzinger and al., OSSYM 2023 [https://ca-roll.github.io/downloads/owler.pdf](https://ca-roll.github.io/downloads/owler.pdf)
- **Presentation: StormCrawler and OpenSearch** (Feb 2023) [https://www.youtube.com/watch?v=azHYI9pnjos](https://www.youtube.com/watch?v=azHYI9pnjos)
- **Crawling the German Health Web: Exploratory Study and Graph Analysis**, Zowalla R, Wetter T, Pfeifer D. J Med Internet Res 2020;22(7):e17853 [https://www.jmir.org/2020/7/e17853](https://www.jmir.org/2020/7/e17853)
- **Tutorial: StormCrawler 1.16 + Elasticsearch 7.5.0** [https://youtu.be/8kpJLPdhvLw](https://youtu.be/8kpJLPdhvLw)
- **StormCrawler open source web crawler strengthened by Elasticsearch, Kibana** [https://www.elastic.co/blog/stormcrawler-open-source-web-crawler-strengthened-by-elasticsearch-kibana](https://www.elastic.co/blog/stormcrawler-open-source-web-crawler-strengthened-by-elasticsearch-kibana)
- **Harvesting Online Health Information**, Richard Zowalla [slides](https://www.slideshare.net/secret/v8Y0qFlGBk7IbB)
- ***Tutorial: StormCrawler 1.10 + Apache SOLR 4.7.0*** [https://youtu.be/F8nvGj03XLo](https://youtu.be/F8nvGj03XLo)
...
- ***DigitalPebble’s Blog: Crawl dynamic content with Selenium and StormCrawler*** [https://digitalpebble.blogspot.co.uk/2017/04/crawl-dynamic-content-with-selenium-and.html](https://digitalpebble.blogspot.co.uk/2017/04/crawl-dynamic-content-with-selenium-and.html)
- ***The Battle of the Crawlers: Apache Nutch vs. StormCrawler*** [https://dzone.com/articles/the-battle-of-the-crawlers-apache-nutch-vs-stormcr](https://dzone.com/articles/the-battle-of-the-crawlers-apache-nutch-vs-stormcr)
- ***Tutorial: StormCrawler + Elasticsearch + Kibana*** [https://digitalpebble.blogspot.co.uk/2017/04/video-tutorial-stormcrawler.html](https://digitalpebble.blogspot.co.uk/2017/04/video-tutorial-stormcrawler.html)
- ***Q&A with InfoQ*** [https://www.infoq.com/news/2016/12/nioche-stormcrawler-web-crawler](https://www.infoq.com/news/2016/12/nioche-stormcrawler-web-crawler)
- ***DigitalPebble’s Blog: Index the web with StormCrawler (revisited)*** [https://digitalpebble.blogspot.co.uk/2016/09/index-web-with-stormcrawler-revisited.html](https://digitalpebble.blogspot.co.uk/2016/09/index-web-with-stormcrawler-revisited.html)
- ***DigitalPebble’s Blog: Index the web with AWS CloudSearch*** [https://digitalpebble.blogspot.co.uk/2015/09/index-web-with-aws-cloudsearch.html](https://digitalpebble.blogspot.co.uk/2015/09/index-web-with-aws-cloudsearch.html)
